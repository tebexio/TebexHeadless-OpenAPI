...
class Category(BaseModel):
    ...
    tiered: Optional[StrictBool] = Field(default=None, description="True if this is a tiered category")
    active_tier: Optional[Tier] = Field(default=None, description="If this is a tiered category and the usernameId is provided, this will be the active tier information for this category.")
    description: Optional[StrictStr] = Field(default=None, description="HTML description of the category")
    packages: Optional[List[Package]] = Field(default=None, description="Packages within the category. For dynamic categories, packages use the `DynamicPackage` shape and are only populated when a `basketIdent` is provided.")
    ...
    image_url: Optional[StrictStr] = Field(default=None, description="URL of the category image, if set.")
    dynamic: Optional[StrictBool] = Field(default=None, description="True if this is a dynamic category. Dynamic categories are populated with custom packages per-basket using the Add Dynamic Packages endpoint, and their packages can only be fetched by providing a `basketIdent`.")
    __properties: ClassVar[List[str]] = ["id", "name", "slug", "parent", "tiered", "active_tier", "description", "packages", "order", "display_type", "image_url", "dynamic"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of active_tier
        if self.active_tier:
            _dict['active_tier'] = self.active_tier.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "active_tier": Tier.from_dict(obj["active_tier"]) if obj.get("active_tier") is not None else None,
